                    ...
                    >
                        ...
                        <template v-slot:action="{ text, record }">
                            <a-space size="small">
                                ...
                                <a-popconfirm cancel-text="否" title="确定删除此文档及其子文档？" ok-text="是"
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const handleQuery = () => {
                ...
                level1.value = [];
                axios.get("/api/doc/getAllDocInfo/" + route.query.ebookId).then((response) => {
                    loading.value = false;
            ...
            const handleSave = () => {
                ...
                axios.post("/api/doc/saveDocData", doc.value).then((response) => {
            ...
            const handleQueryContent = () => {
                axios.get("/api/content/getDocContent/" + doc.value.id).then((response) => {
                    console.log("doc.value.id",doc.value.id)
                    const data = response.data;
                    console.log(data)
                    if (data.success) {
                        console.log("data.data.content", data.data[0].content)
            ...
            const handleDelete = (id: number) => {
                ...
                // console.log(ids)
                axios.delete("/api/doc/deleteDocData/" + deleteIds.join(",")).then((response) => {
                    const data = response.data; // data = commonResp
                    if (data.success) {
                        message.success("删除文档节点成功")
                        // 重新加载列表
                        handleQuery();
                    } else {
                        message.error(data.message);
